...
| 재료 | 평균 절대 소비오차, WAPE, 평균 일수오차, 부족위험일 | `최근 검증 오차 ±N일`                       |
...
- 메뉴 예측 카드: `평균 2.3개 오차`
- 재료 예측 카드: `최근 검증 오차 ±1.3일`

여기서 `+`는 실제 매출이 예측보다 더 나왔다는 뜻이고, `-`는 실제 매출이 예측보다 덜 나왔다는 뜻입니다. 미래 예측 카드에서는 사용자가 예측값을 다시 보정하지 않도록 방향성 문구를 빼고, 최근 검증 오차는 참고 범위로만 표시합니다.
