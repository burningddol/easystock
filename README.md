...
이지스톡의 예측은 “최근 며칠 평균” 하나로 끝내지 않습니다. 실제 구현은 **메뉴 수요 예측 → 옵션 선택률 반영 → 재료 소요량 환산 → 최근 오차 보정 → 소진일/발주 판단 → 백테스트 정확도 표시** 흐름으로 연결됩니다. 핵심 구현 단일 출처는 [forecast.ts](/Users/yamon/Desktop/projects/ezstock/src/lib/domain/forecast.ts)와 [inventory.ts](/Users/yamon/Desktop/projects/ezstock/src/lib/application/inventory.ts)입니다.
...
- 최근 14일 백테스트로 `실제 판매량 ÷ 예측 판매량` 비율을 계산해 메뉴별 보정계수를 적용합니다.
- 메뉴 보정은 비교 가능한 실제 판매일 7일 이상, 실제 판매 총량 10개 이상일 때만 적용합니다.
- 예측과 실제 차이가 5% 이내면 노이즈로 보고 보정하지 않습니다.
- 메뉴 보정계수는 `0.80~1.25`로 제한하고 DB에 누적 저장하지 않습니다.
- 매출 예측은 별도 매출 보정계수를 곱하지 않고, 보정된 메뉴 예측 수량 × 현재 메뉴 가격으로 계산합니다.
...
재료 소진일은 현재 재고에서 미래 일별 예상 소비량을 하루씩 차감해 계산합니다. 보정된 메뉴 기반 재료 수요가 있으면 단순 소비 이력보다 이를 우선 사용하고, 최근 실제 소비와의 차이를 한 번 더 안전하게 보정합니다.

최근 오차 보정:

- 최근 14일 백테스트로 `실제 재료 소비량 ÷ 메뉴 기반 예측 소비량` 비율을 계산합니다.
- 비교 가능한 실제 소비일이 7일 미만이면 보정하지 않습니다.
- 예측과 실제 차이가 5% 이내면 노이즈로 보고 보정하지 않습니다.
- 보정계수는 `0.85~1.30`으로 제한합니다. 과거 오차를 그대로 누적 반영하지 않고, 현재 조회 시점의 최근 백테스트로만 계산합니다.
- 이 보정은 미래 일별 재료 수요에만 적용한 뒤, 소진일·발주 추천량을 다시 계산합니다.
...
운영 화면의 예측값은 최근 검증 결과를 참고한 내부 보정이 이미 반영된 최종값입니다. 정확도 화면의 오차율은 예측값을 한 번 더 조정하라는 뜻이 아니라, 최근 최종 예측이 실제와 얼마나 차이 났는지 확인하는 검증 지표입니다.
...
| 매출 | 평균 절대 금액오차, WAPE, signed error              | `평균 N만원 오차`, 과거 비교에서 `+/-` 방향       |
| 재료 | 평균 절대 소비오차, WAPE, 평균 일수오차, 부족위험일 | 기본 카드에는 소진일만 표시, 정확도 화면에서 검증 |
...
여기서 `+`는 실제 매출이 예측보다 더 나왔다는 뜻이고, `-`는 실제 매출이 예측보다 덜 나왔다는 뜻입니다. 미래 예측 카드에서는 사용자가 예측값을 다시 보정하지 않도록 오차 범위와 방향성 문구를 빼고, 검증 지표는 예측 정확도 화면에서만 확인합니다.
