-- Migration: block negative stock on initial sale save
--
-- Aligns save_sale with edit_sale so both reject insufficient ingredient stock
-- instead of silently clamping stock to zero.
...
comment on function public.save_sale(date, jsonb) is
  '판매 트랜잭셔널 저장 (FR-006). 신규 저장도 재료 부족 시 negative_stock으로 차단한다.';
