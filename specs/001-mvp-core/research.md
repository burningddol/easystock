...
- 메뉴 수요 예측도 최근 14일 actual/predicted 총량 비율로 메뉴별 보정계수를 적용한다
- 메뉴 보정은 비교 가능일 7일 이상, 실제 판매 총량 10개 이상일 때만 적용하고, 5% 이내 차이는 노이즈로 무시한다
- 메뉴 보정계수는 `0.80~1.25`로 clamp한다
- 매출 예측은 별도 매출 보정계수를 두지 않고, 보정된 메뉴 수량에 현재 가격을 곱해 산출한다
...
- 매출에 별도 보정을 또 곱하지 않는 이유는 이중보정 위험 때문이다. 메뉴 수량 보정이 매출과 재료 수요의 공통 원천이므로, 매출은 보정된 메뉴 수량의 결과로만 개선한다
...
- 매출 총액 보정계수 추가: 단기적으로 총매출 오차는 낮출 수 있지만, 메뉴별 수량과 재료 소요량 설명력이 깨지고 메뉴 보정과 중복될 수 있어 보류
